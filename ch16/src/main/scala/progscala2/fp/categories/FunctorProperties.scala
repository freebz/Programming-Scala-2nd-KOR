// src/main/scala/progscala2/fp/categories/FunctorProperties.scala
...
class FunctorProperties extends FunSpec with PropertyChecks {
  ...
  def id[A] = identity[A] _       // 단위원 메서드를 함수로 끌어올린다.
  ...
  describe ("Functor identity maps between the identities of the categories") {
    ...
    it ("works for Function Functors") {
      val f1: Int => Int = _ * 2
      def id[A] = identity[A] _ // 메서드를 함수로 끌어올린다.
      import FunctionF._
      forAll { (i: Int) =>
